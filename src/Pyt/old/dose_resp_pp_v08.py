...
def model_POF(Y=None, T=None, D=None, Z=None, X=None,
    ...
    if C and (country_id is not None):
        ...
        gamma_for_obs = jnp.tile(gamma_global, (N, 1))          # (N, Kz)
        
    omega_global = numpyro.sample("omega_global", dist.Normal(0., 1.))
    if P and (project_id is not None):
        sigma_omega_p = numpyro.sample("sigma_omega_p", dist.HalfNormal(1.0))
        with numpyro.plate("project", P):
            omega_off = numpyro.sample("omega_off", dist.Normal(0., 1.))
        omega_p = omega_global + sigma_omega_p * omega_off
        omega_for_obs = omega_p[project_id]
    else:
        omega_for_obs = jnp.repeat(omega_global, N)

    mu_log_eta = numpyro.sample("mu_log_eta", dist.Normal(0., 1.))
    if P and (project_id is not None):
        sigma_log_eta_p = numpyro.sample("sigma_log_eta_p", dist.HalfNormal(1.0))
        with numpyro.plate("project", P):
            eta_off = numpyro.sample("eta_off", dist.Normal(0., 1.))
        log_eta_p = mu_log_eta + sigma_log_eta_p * eta_off
        eta_for_obs = jnp.exp(log_eta_p[project_id])
    else:
...
try:
    df1 = pd.read_csv('C:/Users/DaSchulz/OneDrive - European Forest Institute/Dokumente/research/redd_intensity/data/pd_medium_ALL.csv')
    #df2 = pd.read_csv('C:/Users/DaSchulz/OneDrive - European Forest Institute/Dokumente/research/redd_intensity/data/pd_medium_ALL.csv')
...
# relevant: all of above
relevant_columns = [Y_name] + X_names + Z_names + [D_name, T_name]
...
village_id = village_id.to_numpy().astype(np.int32)
project_id = project_id.to_numpy().astype(np.int32)
country_id = country_id.to_numpy().astype(np.int32)
...
plt.show()
fig.savefig("POF_MCMC_v8_diagnostics_truedata_medium.png", dpi=300)
...
posterior_summary_df.to_csv("POF_MCMC_v8_posteriors_truedata_medium.csv", index=False)
diagnostic_summary_df.to_csv("POF_MCMC_v8_diagnostics_truedata_medium.csv", index=False)
# %% Check summary
...
# %%
# Get the raw, decentered effects
u_raw_level = 'u_v_raw'
sigma_level = 'sigma_v'

# Extract the raw effects and the standard deviation
u_raw_0 = posterior[u_raw_level][:,:,0].values.flatten()
sigma_v = posterior[sigma_level].values.flatten()

plt.figure(figsize=(6,5))
plt.scatter(u_raw_0, sigma_v, alpha=0.3)
plt.ylabel("sigma (population sd)")
plt.xlabel("u_raw[0] (standardized village effect)")
plt.title("Decentered parameterization diagnostic")
plt.show()
# %%
