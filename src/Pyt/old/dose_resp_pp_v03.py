# Version 0.3
# Problems in v0.2: 
# - T is beta-distributed, but independent of Z.
# Solution: Model T as function of Z.
...
def model_POF(Z, X, T, Y=None, rng_key=None, funcForm="linear"):
    ...
    # Version 0.3: Model T with a dosage propensity based on Z
    # T is now a function of Z
    
    # Coefficients for the treatment assignment model
    gamma = numpyro.sample("gamma", dist.Normal(0., 1).expand([Z.shape[1]]))
    
    # Linear combination of Z
    logits = Z @ gamma
    
    # The parameters of the Beta distribution are alpha and beta, not beta_1 and beta_2 as in the original code.
    alpha_param = numpyro.deterministic("alpha_param", jnp.exp(logits)) 
    beta_param = numpyro.sample("beta_param", dist.Exponential(1)) 
    
    numpyro.sample("T", dist.Beta(alpha_param, beta_param), obs=T)
...
def data_generating(rng_key,
    ...
    """
    # Split the rng_key once at the beginning for all random processes
    rng_key, rng_key_, key_split_X, key_split_Z, key_split_T = random.split(rng_key, 5)
    ...
    # --- New code for generating T based on Z ---
    # Define true coefficients for the treatment assignment model
    gamma_true = jnp.array([1, -0.5, 0.3, -0.8, 0.2], dtype='float32') 
    
    # Calculate the alpha_param from Z and gamma_true
    logits = Z @ gamma_true
    alpha_param = jnp.exp(logits)
    
    # Fix the second parameter for the Beta distribution
    beta_param = jnp.array(4.0, dtype='float32')
    
    # Sample T from the Beta distribution
    T = dist.Beta(alpha_param, beta_param).sample(key=key_split_T)
    ...
    coefTrue['beta_param'] = beta_param
...
# plot T vs Z
plt.figure(figsize=(8, 6))
sns.scatterplot(x=test[5][:, 1], y=test[4])
plt.title('Treatment Variable (T) vs. Z')
plt.xlabel('Z')
...
plt.savefig(f'POF_MCMC_v03.png', dpi=300)
plt.show()
...
#all_posterior_summaries.to_csv('post_sum_v03.csv')
