...
def model_POF(Y=None, T=None, D=None, Z=None, X=None,
    ...
    sigma_v = numpyro.sample("sigma_v", dist.HalfNormal(1.0)) if V > 0 else 0.0
    u_v = numpyro.sample("u_v", dist.Normal(0, sigma_v).expand([V])) if V > 0 else jnp.array([0.0])

    sigma_p = numpyro.sample("sigma_p", dist.HalfNormal(1.0)) if P > 0 else 0.0
    u_p = numpyro.sample("u_p", dist.Normal(0, sigma_p).expand([P])) if P > 0 else jnp.array([0.0])

    sigma_c = numpyro.sample("sigma_c", dist.HalfNormal(1.0)) if C > 0 else 0.0
    u_c = numpyro.sample("u_c", dist.Normal(0, sigma_c).expand([C])) if C > 0 else jnp.array([0.0])

    # add effects only if ids are provided
    v_eff = u_v[village_id] if village_id is not None else 0.0
    p_eff = u_p[project_id] if project_id is not None else 0.0
    c_eff = u_c[country_id] if country_id is not None else 0.0
...
def data_generating(rng_key, funcForm="linear", N=1000, K=5, V=20, P=10, C=5):
    rng_key, kX, kZ, kD, kT, kVill, kProj, kCoun, kv, kp, kc = random.split(rng_key, 11)
    ...
    if V is None or V == 0:
        ...
        sigma_v_true = 0.0
        village_v = jnp.zeros((V,), dtype=jnp.float32)
    else:
        village_id = random.randint(kVill, shape=(N,), minval=0, maxval=V)
        sigma_v_true = 0.8
        village_v = dist.Normal(0., sigma_v_true).sample(kv, (V,))

    # save into coefTrue consistently
    coefTrue['sigma_v'] = float(sigma_v_true)

    u_effect_v = village_v[village_id]
    ...
    if P is None:
        ...
        sigma_p_true = 0.5
        project_u = dist.Normal(0., sigma_p_true).sample(kp,(P,))
    
    coefTrue['sigma_p'] = sigma_p_true
    ...
    if C is None:
        ...
        sigma_c_true = 0.3
        country_u = dist.Normal(0., sigma_c_true).sample(kc,(C,))
    
    coefTrue['sigma_c'] = sigma_c_true
    ...
    gamma_true = jnp.array([0.9, -0.05, 0.04, 0.03, -0.02])
    omega_true = 1.
    lam = jnp.exp(jnp.dot(Z, gamma_true) + omega_true * D)
    eta_true = 2.5
    T_counts = dist.NegativeBinomial2(lam, eta_true).sample(key=kT)

    coefTrue['gamma'] = gamma_true
    coefTrue['omega'] = omega_true
    coefTrue['eta'] = eta_true

    # --- scaled version
    T_counts_f = T_counts.astype(jnp.float32)
    T_max = jnp.max(T_counts_f)
    T_cont = T_counts_f / T_max
    ...
    if funcForm == "homogenous":
        b1 = 0.5
        tau_true = b1 * D
        # update coefTrue with new parameters
    ...
    model_kwargs = {
        "Z": Z,
        "X": X,
        "T": T_counts,        # raw counts; model rescales internally
        "D": D,
        "Y": Y_scaled,        # << was Y_unscaled
        "village_id": village_id,
        "project_id": project_id,
        "country_id": country_id,
        # pass sizes so plates are active
...
Y_scaled, Y_unscaled, Y0, T_counts, T_cont, D, Z, X, village_id, project_id, country_id, tau_true, T_leg, model_kwargs, coefTrue, Y_mean, Y_std = data_generating(
    rng_key, funcForm="linear", N=N, K=K, V=V, P=P, C=C
)

df = pd.DataFrame({"T": np.array(T_counts), "D": np.array(D)})
...
# %%
# inspect project_id
# table of counts of project_id per country
project_counts = df1[project_column].value_counts()
country_project_counts = df1.groupby(country_column)[project_column].nunique()
print("Project counts per country:")
print(country_project_counts)
